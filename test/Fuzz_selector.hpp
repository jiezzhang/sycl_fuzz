...
class fuzz_selector : public sycl::device_selector {
public:
  ...
  int score(sycl::info::device_type type, sycl_dev_type desired) const {
    int result = -1;
  ...
  virtual int operator()(const sycl::device &dev) const override {
    int result = -1;

    auto vendor = dev.get_platform().get_info<sycl::info::platform::name>();
